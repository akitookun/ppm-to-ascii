...
pub const Error = error{ InvalidScale, InvalidPrintType };

pub const AsciiImage = struct {
    width: usize,
    height: usize,
    chars: []u8,
    colors: []u8
...
pub const font8x8_basic: [128][16]u8 = .{
...
const ascii_table =
" .'`^\",:;Il!i<>~+_-?][}{1)(|\\/tfjrxnuvczXYUJCLQ0OZmwqpd bkh a o*#MW&8%B@$";

pub fn luminance(r: u8, g: u8, b: u8) u8 {
    return @intCast((@as(u32, r)*2126 + @as(u32,g)*7152 + @as(u32,b)*722)/10000);
}

pub fn convert(image: ppm.PpmImage, allocator: std.mem.Allocator, scale: f32) !AsciiImage {
    if (scale <= 0.0) return Error.InvalidScale;

    const width: usize = @intFromFloat(@as(f32, @floatFromInt(image.width))*scale);
    const height: usize = @intFromFloat(@as(f32, @floatFromInt(image.height))*scale/2.0);

    const chars = try allocator.alloc(u8, width * height);
    const colors = try allocator.alloc(u8, width * height * 3);

    fillAscii(image, chars, colors, width, height, scale);
    return AsciiImage{ .width=width, .height=height, .chars=chars, .colors=colors };
}

fn fillAscii(image: ppm.PpmImage, chars: []u8, colors: []u8,
    out_w: usize, out_h: usize, scale: f32) void {
    const step = 1.0/scale;
    for (0..out_h) |y| {
        const src_y = @min(@as(usize, @intFromFloat(@as(f32, @floatFromInt(y)) * step * 2.0)), image.height - 1);
        for (0..out_w) |x| {
            const src_x = @min(@as(usize, @intFromFloat(@as(f32, @floatFromInt(x)) * step)), image.width - 1);
            writePixel(image, chars, colors, out_w, x, y, src_x, src_y);
        }
    }
}

fn writePixel(image: ppm.PpmImage, chars: []u8, colors: []u8,
    out_w: usize, x: usize, y: usize, src_x: usize, src_y: usize) void {
    const idx = (src_y*image.width+src_x)*3;
    const r = image.pixels[idx+0];
    const g = image.pixels[idx+1];
    const b = image.pixels[idx+2];

    const lum = luminance(r,g,b);
    const char_idx = (lum*(ascii_table.len-1))/255;
    const out_idx = y*out_w + x;

    chars[out_idx] = ascii_table[char_idx];
    colors[out_idx*3 + 0] = r;
    colors[out_idx*3 + 1] = g;
    colors[out_idx*3 + 2] = b;
}

pub fn print(image: AsciiImage, mode: []const u8) !void {
    ...
    for (0..image.height) |y| {
        for (0..image.width) |x| {
            ...
            try printCell(image, idx, mode, out);
        }
        try out.print("\n", .{});
    }
    try out.flush();
...
fn printCell(image: AsciiImage, index: usize, mode: []const u8, out: *std.io.Writer) !void {
    const c = image.chars[index];
    const color_idx = index*3;
    const r = image.colors[color_idx+0];
    const g = image.colors[color_idx+1];
    const b = image.colors[color_idx+2];

    if (std.mem.eql(u8, mode, "normal")) {
        try out.print("{c}", .{c});
        return;
    }
    if (std.mem.eql(u8, mode, "color")) {
        try out.print("\x1b[38;2;{};{};{}m{c}\x1b[0m", .{ r,g,b,c });
        return;
    }
    if (std.mem.eql(u8, mode, "full_color")) {
        try out.print("\x1b[48;2;{};{};{}m \x1b[0m", .{ r,g,b });
        return;
    }
    return Error.InvalidPrintType;
}
